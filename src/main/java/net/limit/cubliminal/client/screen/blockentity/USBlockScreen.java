package net.limit.cubliminal.client.screen.blockentity;
...
public class USBlockScreen extends Screen {
    ...
    protected void init() {
        ...
        this.buttonEntities = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(!this.structureBlock.shouldIgnoreEntities()).omitKeyText().build(this.width / 2 + 4 + 100, 160, 50, 20, INCLUDE_ENTITIES_TEXT, (button, includeEntities) -> {
            this.structureBlock.setIgnoreEntities(!includeEntities);
        }));
        this.buttonMirror = this.addDrawableChild(CyclingButtonWidget.builder(BlockMirror::getName).values(BlockMirror.values()).omitKeyText().initially(this.mirror).build(this.width / 2 - 20, 185, 40, 20, Text.literal("MIRROR"), (button, mirror) -> {
            this.structureBlock.setMirror(mirror);
        }));
        this.buttonShowAir = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.structureBlock.shouldShowAir()).omitKeyText().build(this.width / 2 + 4 + 100, 80, 50, 20, SHOW_AIR_TEXT, (button, showAir) -> {
            this.structureBlock.setShowAir(showAir);
        }));
        this.buttonShowBoundingBox = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.structureBlock.shouldShowBoundingBox()).omitKeyText().build(this.width / 2 + 4 + 100, 80, 50, 20, SHOW_BOUNDING_BOX_TEXT, (button, showBoundingBox) -> {
    ...
    static {
        MODES_EXCEPT_DATA = MODES.stream().filter((mode) -> {
